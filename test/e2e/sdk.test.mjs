...
test("the built SDK completes a real HTTP round trip through its public API", async (context) => {
  const requests = [];
  const payload = { amount: 100, base: "USD", date: "2026-08-14", rates: { JPY: 14850 } };
  ...
  const result = await esm.default.exchangeRates.getLatestRates({
    amount: 100,
    base: "USD",
    symbols: "JPY,CNY",
  });
  ...
  assert.equal(requests[0].url, "/v1/latest?amount=100&base=USD&symbols=JPY%2CCNY");

  const require = createRequire(import.meta.url);
  const cjs = require(resolve(repositoryRoot, "dist/index.js"));
  assert.equal(cjs.default, cjs.currencyExchangeClient);
});
...
test("the CLI and npm package surface are publishable", async () => {
  ...
  assert.match(help, /pontx-frankfurter/);
